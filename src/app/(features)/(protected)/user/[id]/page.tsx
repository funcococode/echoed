  ...
  return (
    <section className="mx-auto max-w-5xl space-y-8 px-4 py-6 sm:px-6">
      ...
      <motion.div
        ...
        className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:bg-neutral-900 dark:border-neutral-800 shadow-sm"
      >
        {/* soft cover bar */}
        <div className="h-20 " />
        ...
        <div className="flex flex-col gap-5 px-6 pb-6 pt-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {data?.username && (
                  <span className="truncate text-sm text-slate-500 dark:text-neutral-400">(@{data.username})</span>
                )}
              </div>
              {data?.email && (
                <div className="mt-0.5 text-sm text-slate-500 dark:text-neutral-400">{data.email}</div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-700 transition-colors hover:bg-slate-50 dark:text-neutral-400 dark:hover:bg-neutral-950  cursor-pointer"
            >
        ...
        {/* Stats */}
        <div className="border-t border-gray-200 bg-slate-50/60 dark:border-neutral-800 dark:bg-neutral-950 px-6 py-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
            {stats.map((s) => (
              <motion.div
                ...
                className="rounded-xl border border-gray-200 bg-white dark:bg-neutral-900 dark:border-neutral-800 px-4 py-3 shadow-sm"
              >
                <div className="flex items-center justify-between">
                  <div className="text-xs font-medium tracking-wide text-slate-500 dark:text-neutral-400">{s.label}</div>
                  <div className="text-slate-400 dark:text-neutral-400">{s.icon}</div>
                </div>
                <div className="mt-1 text-lg font-semibold text-slate-900 dark:text-neutral-100">{s.value}</div>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        {/* Loading bar */}
        {loading && (
          <div className="h-0.5 w-full overflow-hidden bg-gray-100 dark:bg-neutral-950">
            <motion.div
              initial={{ x: '-100%' }}
              animate={{ x: '100%' }}
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-neutral-200">
            <span className="inline-block h-4 w-1 rounded bg-slate-800/80 dark:bg-neutral-400" />
            Echoes
            <span className="rounded-md border border-gray-200 bg-white dark:bg-neutral-950 dark:border-neutral-800 dark:text-neutral-400 px-2 py-0.5 text-xs text-slate-600 shadow-sm">
        ...
        {!loading && !err && posts?.length === 0 && (
          <div className="grid place-items-center rounded-2xl border border-gray-200 bg-white dark:bg-neutral-950 dark:border-neutral-800 dark:text-neutral-400 p-10 text-center shadow-sm">
            <div className="mb-3 grid h-12 w-12 place-items-center rounded-2xl border border-gray-200 bg-slate-50 dark:bg-neutral-900 dark:border-neutral-800 dark:text-neutral-400 text-slate-500">
              <TbNotes className="h-5 w-5" />
            </div>
            <div className="text-sm font-medium text-slate-800 dark:text-neutral-400">No Echoes yet</div>
