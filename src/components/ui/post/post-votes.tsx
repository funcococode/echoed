...
export default function PostVotes({post} : PostVotesProps) {
    ...
    return (
        currentPost ? <div className={`flex flex-col items-center gap-2 `}>
            <VoteUpButton handlerFn={handleClick} isVoted={currentPost?.votePositive && currentPost?.votedByMe} />
                <p className={`text-xs ${currentPost.votedByMe && (currentPost.votePositive ? 'text-indigo-600' : 'text-red-500')}`}>
